...
from sqlalchemy import BigInteger, Column, DateTime, MetaData, Table, UniqueConstraint
...
def unique_constraint(*columns: str, name: str | None = None) -> UniqueConstraint:
    """创建与 deleted_at 的组合唯一约束，默认约束名为 uq_<col1>_<col2>_..._deleted_at"""
    all_columns = (*columns, "deleted_at")
    constraint_name = name or "uq_" + "_".join(all_columns)
    return UniqueConstraint(*all_columns, name=constraint_name)
...
class BaseRepositoryMysql(ABC, Generic[T]):
    ...
    async def select_one(self, *conditions):
        stmt = self.tl.select().where(*conditions, self.tl.c.deleted_at == 0).limit(1)
    ...
    async def select_many(
        ...
        *conditions,
        ...
        stmt = self.tl.select().where(*conditions, self.tl.c.deleted_at == 0)
    ...
    async def select_count(self, *conditions):
        stmt = (
            select(func.count())
            .select_from(self.tl)
            .where(*conditions, self.tl.c.deleted_at == 0)
        )
        return (await self.conn.execute(stmt)).scalar_one()

    async def select_exists(self, *conditions):
        stmt = select(exists().where(*conditions, self.tl.c.deleted_at == 0))
